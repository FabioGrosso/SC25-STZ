...
double* ZFP_compress(void* array, size_t nx, size_t ny, size_t nz, double abs_error_bound, size_t* compressed_size) {
    ...
    *compressed_size = zfpsize;
    ...
    double* decompressed_array = (double*)malloc(nx * ny * nz * sizeof(double));
...
int main() {
    ...
    // DataArray oriData = readBinaryData("/N/u/daocwang/BigRed200/data/miranda_1024x1024x1024_double32.raw");
    // DataArray oriData = readBinaryData("/N/u/daocwang/BigRed200/data/magnetic_reconnection_512x512x512_double32.raw");
    ...
    double* data = ZFP_compress(oriData, SIZE_X, SIZE_Y, SIZE_Z, eb, &compressed_size);
    ...
    std::cout << "[ZFP] Compression Ratio: " << (TOTAL_SIZE * sizeof(double)) / (double)compressed_size << std::endl;
